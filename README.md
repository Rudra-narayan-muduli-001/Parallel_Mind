...
| **Failover** | Immediate failover — no delay between candidate attempts |
...
cp .env.example .env
# Edit .env — fill in keys for the providers you use
...
| `config/routing_table.py` | 5-tier routing policies per task type for `RuleBasedPolicy` |
| `config/effort_presets.py` | Low/High effort generation parameters (temperature, max_tokens, timeout) |
...
```
Topic  ──►  Planner (LLM decomposition)  ──►  Parallel Researcher Agents  ──►  LLM Synthesis  ──►  Report
```
...
```
Path  ──►  Splitter (file discovery + tier assignment)  ──►  Parallel Reviewer Agents  ──►  Dedupe Merge  ──►  Report
```
...
| `VotingAggregator` | Picks most common output across candidates | Future use |
